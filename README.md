...
- ⚖️ **Lawyer filing** — encrypt PDF, pin to IPFS, register metadata on Sepolia, grant access
- 👨‍⚖️ **Judge workspace** — register judicial orders against case IDs
- 🏛️ **Citizen vault** — load by case ID, verify integrity, authorized download
...
- **Java 17+** (Auth / Gateway); *Java 21 for Audit & Notifications if required*
- **Python 3.12+**
- **Node.js 18+**
- **MySQL 8**
...
1. **Configure Environment:**
   ...
   PINATA_JWT=your_jwt_here # or 'mock' for local-only IPFS
   ...
   ```
2. **Start Services:**
   Follow the full start order in **[START_ALL.md](./START_ALL.md)** (gateway **last**, then frontend).
   *(Windows users can use `start-all.ps1` where available).*
3. **Launch the App:**
   Open **[http://localhost:3000](http://localhost:3000)** → connect MetaMask → complete identity binding → use Documents / Orders / My vault.
...
| ⛓️ Blockchain | [http://localhost:8083/blockchain/health](http://localhost:8083/blockchain/health) |
...
<details>
<summary><b>🛠️ Config Cheat Sheet</b> (Click to expand)</summary>
...
<details>
<summary><b>💡 Demo Notes</b> (Click to expand)</summary>
